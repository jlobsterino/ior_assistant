...
def load_meta(path=path_to_load):
    ...
    global documents, doc_sids, id_to_index, incident_ids, incident_dates
...
def build_and_cache_small_index(session_id: str, target_ids: list, id_to_text_map: dict) -> bool:
    ...
    for doc_id in target_ids:
        if doc_id in id_to_index:
            ...
            valid_indices.append(id_to_index[doc_id])

    skipped_count = len(target_ids) - len(valid_ids)
    print(f"[SMALL_FAISS] Всего ID передано: {len(target_ids)}, успешно сопоставлено: {len(valid_indices)}, пропу...")
